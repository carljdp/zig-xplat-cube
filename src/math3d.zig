// copied from: https://github.com/andrewrk/tetris/blob/73fad7bc42777637e1f67374d5cb4dfe62b27e0c/src/math3d.zig
...
// test "ShouldFail" {
//     // std.debug.print("(this one is supposed to fail..)\n", .{});
...
//     const not_a = a.scale(0.141603, 0.717297, 0.635712);
//     _ = not_a;

//     try assert_matrix_eq(a, a);
// }

test "scale" {
    // std.debug.print("Running test case 'Scale'\n", .{});
    ...
    try assert_matrix_eq(answer, expected);
}

test "translate" {
    // std.debug.print("Running test case 'Translate'\n", .{});
    const m = Mat4x4{
        .data = [_][4]f32{
            [_]f32{ 0.840188, 0.911647, 0.277775, 0.364784 },
            [_]f32{ 0.394383, 0.197551, 0.55397, 0.513401 },
...
test "ortho" {
    // std.debug.print("Running test case 'Ortho'\n", .{});
    ...
    try assert_matrix_eq(m, expected);
}

// GPT generated
// When comparing two f32 values in a testing scenario, it is often good to use
// a combination of both relative and absolute tolerances, as floating-point
// arithmetic can have imprecisions.
fn expectApproxEqF32(a: f32, b: f32) !void {
    // Absolute Tolerance (epsilon): Since coordinates can be near zero,
    // especially when working with normalized vectors, using a small absolute
    // tolerance like 1.0e-6 can help catch those cases.
    const epsilon = 1.0e-2; // probably due to low precision used in calculations ??
    // Relative Tolerance: For scaling or transformations where large values
    // might be involved, a relative tolerance like 1.0e-6 can help catch
    // proportional inaccuracies.
    const relativeTolerance = 1.0e-2; // probably due to low precision used in calculations ??

    try std.testing.expectApproxEqAbs(a, b, epsilon);
    try std.testing.expectApproxEqRel(a, b, relativeTolerance);
}

test "expectApproxEq" {
    // should pass if epsilon / relativeTolerance is set to 1.0e-2
    try expectApproxEqF32(0.000001, 0.00000101);
}

fn assert_matrix_eq(left: Mat4x4, right: Mat4x4) !void {
    try expectApproxEqF32(left.data[0][0], right.data[0][0]);
    try expectApproxEqF32(left.data[0][1], right.data[0][1]);
    try expectApproxEqF32(left.data[0][2], right.data[0][2]);
    try expectApproxEqF32(left.data[0][3], right.data[0][3]);

    try expectApproxEqF32(left.data[1][0], right.data[1][0]);
    try expectApproxEqF32(left.data[1][1], right.data[1][1]);
    try expectApproxEqF32(left.data[1][2], right.data[1][2]);
    try expectApproxEqF32(left.data[1][3], right.data[1][3]);

    try expectApproxEqF32(left.data[2][0], right.data[2][0]);
    try expectApproxEqF32(left.data[2][1], right.data[2][1]);
    try expectApproxEqF32(left.data[2][2], right.data[2][2]);
    try expectApproxEqF32(left.data[2][3], right.data[2][3]);

    try expectApproxEqF32(left.data[3][0], right.data[3][0]);
    try expectApproxEqF32(left.data[3][1], right.data[3][1]);
    try expectApproxEqF32(left.data[3][2], right.data[3][2]);
    try expectApproxEqF32(left.data[3][3], right.data[3][3]);
}

test "mult" {
    // std.debug.print("Running test case 'Mult'\n", .{});
    const m1 = Mat4x4{
        .data = [_][4]f32{
    ...
    try assert_matrix_eq(tmp, answer);
}

test "rotate" {
    // std.debug.print("Running test case 'Rotate'\n", .{});
    const m1 = Mat4x4{
        .data = [_][4]f32{
            [_]f32{ 0.840188, 0.911647, 0.277775, 0.364784 },
            [_]f32{ 0.394383, 0.197551, 0.55397, 0.513401 },
    ...
    try assert_matrix_eq(actual, expected);
}
